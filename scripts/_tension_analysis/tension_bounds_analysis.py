...
try:
    # Try to find MCMC results
    mcmc_files = list(RESULTS_DIR.glob('cgc_mcmc_chains_*.npz'))
    if mcmc_files:
        mcmc_path = max(mcmc_files)  # Get most recent
        mcmc = np.load(mcmc_path, allow_pickle=True)
    else:
        mcmc = np.load(RESULTS_DIR / 'cgc_mcmc_chains_20260201_131726.npz', allow_pickle=True)
    chains = mcmc['chains']
