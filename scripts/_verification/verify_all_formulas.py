...
print(f"\nRESULT: n_g = {n_g_derived:.4f} ≈ 0.0125 (THESIS v12 CANONICAL VALUE)")
print("✓ This is the FIXED value from theory (β₀²/4π²)")
print("   Old value n_g = 0.138 was an MCMC-fitted value from earlier versions")
print("   Thesis v12 fixes n_g = 0.0125 and only fits μ_fit = 0.47")
...
def g_z(z, n_g=0.0125, z_trans=1.67):
    """Time evolution function (Thesis v12)"""
    if z > z_trans:
        return 0
    return (1 + z)**(-n_g)
...
def G_eff_ratio(k, z, rho, mu=0.47):
    """G_eff/G_N - 1 (Thesis v12: μ_fit = 0.47)"""
    return mu * (k/k_0)**n_g * g_z(z) * S_rho(rho)

print("\nG_eff/G_N - 1 at various conditions (μ_fit=0.47, Thesis v12):")
conditions = [
...
KEY INSIGHT: The hybrid screening mechanism resolves apparent tension:
  ...
  μ_eff(void) = μ_fit × S_avg ≈ 0.47 × 0.31 = 0.149 (cosmological screening in voids)
  μ_eff(Lyα/IGM) ≈ 6×10⁻⁵ (Chameleon + Vainshtein hybrid in dense IGM)

DWARF GALAXY PREDICTION:
  With μ_eff(void) = 0.149: Δv = +12 ± 3 km/s (void vs cluster)

FORMULAS VERIFIED AS CORRECT (Thesis v12):
