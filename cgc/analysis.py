...
def compute_derived_parameters(chains: np.ndarray) -> Dict[str, np.ndarray]:
    ...
    # H0 = h × 100
    derived['H0'] = chains[:, 2] * 100
    
    # Ω_m = (ω_cdm + ω_b) / h²
    derived['Omega_m'] = (chains[:, 1] + chains[:, 0]) / chains[:, 2]**2
    
    # σ8 (approximate scaling from Planck)
    sigma8_fid = PLANCK_BASELINE['sigma8']
    Omega_m_fid = PLANCK_BASELINE['Omega_m']
    derived['sigma8'] = sigma8_fid * (derived['Omega_m'] / Omega_m_fid)**0.25
...
def compute_tension_metrics(chains: np.ndarray,
    ...
    # ΛCDM tension: Planck vs SH0ES
    combined_H0_err = np.sqrt(planck_H0_err**2 + shoes_H0_err**2)
    H0_tension_lcdm = abs(shoes_H0 - planck_H0) / combined_H0_err
    
    # CGC tension: maximum of CGC vs Planck and CGC vs SH0ES
    tension_cgc_planck = abs(H0_mean - planck_H0) / np.sqrt(H0_std**2 + planck_H0_err**2)
    tension_cgc_shoes = abs(H0_mean - shoes_H0) / np.sqrt(H0_std**2 + shoes_H0_err**2)
    H0_tension_cgc = max(tension_cgc_planck, tension_cgc_shoes)
    ...
    S8_tension_lcdm = abs(planck_S8 - wl_S8) / combined_S8_err
    
    tension_cgc_planck_S8 = abs(S8_mean - planck_S8) / np.sqrt(S8_std**2 + planck_S8_err**2)
    tension_cgc_wl_S8 = abs(S8_mean - wl_S8) / np.sqrt(S8_std**2 + wl_S8_err**2)
    S8_tension_cgc = max(tension_cgc_planck_S8, tension_cgc_wl_S8)
