...
# Base directory - project root (go up 3 levels: cgc -> simulations -> project root)
_THIS_DIR = os.path.dirname(os.path.abspath(__file__))  # simulations/cgc/
_SIMULATIONS_DIR = os.path.dirname(_THIS_DIR)  # simulations/
_BASE_DIR = os.path.dirname(_SIMULATIONS_DIR)  # project root
...
PATHS = {
    ...
    'data_cgc_sim': os.path.join(_SIMULATIONS_DIR, "cgc_data"),
    ...
    'class_cgc': os.path.join(_SIMULATIONS_DIR, "class_cgc"),
...
MCMC_DEFAULTS = {
    # ═══════════════════════════════════════════════════════════════════════
    # Sampler Configuration (PRODUCTION SETTINGS)
    # ═══════════════════════════════════════════════════════════════════════
    
    # Number of walkers (2^k for efficient parallelization)
    'n_walkers': 128,
    
    # Number of MCMC parameters (only cosmological + μ are free)
    ...
    'n_steps_standard': 10000,
    
    # Number of steps (publication quality)
    'n_steps_publication': 50000,
    
    # Burn-in steps (absolute, not fraction)
    'burn_in_test': 500,
    'burn_in_standard': 2000,
    'burn_in_publication': 10000,
    
    # Burn-in fraction (fraction of chain to discard)
    'burn_in_fraction': 0.2,
    
    # Thinning factor (take every nth sample to reduce autocorrelation)
    'thin': 10,
    
    # Checkpoint interval (save progress every N steps)
    'checkpoint_every': 1000,
    
    # Random seed for reproducibility
    'seed': 42,
    
    # Target acceptance rate (optimal for Gaussian targets)
    'target_acceptance': 0.234,
    
    # Affine-invariant stretch move parameter (optimal for multimodal)
    'affine_invariant_a': 2.0,
    
    # ═══════════════════════════════════════════════════════════════════════
    # Proposal Scales (tuned for good acceptance rates)
    # ═══════════════════════════════════════════════════════════════════════
    'proposal_scales': {
        'omega_b': 0.0005,      # ±0.0005 around ~0.0224
        'omega_cdm': 0.003,     # ±0.003 around ~0.120
        'h': 0.008,             # ±0.008 around ~0.674
        'ln10As': 0.02,         # ±0.02 around ~3.045
        'n_s': 0.005,           # ±0.005 around ~0.965
        'tau_reio': 0.008,      # ±0.008 around ~0.054
        'cgc_mu': 0.02,         # ±0.02 around ~0.149
    },
    
    # ═══════════════════════════════════════════════════════════════════════
    # Convergence Criteria (STRICT for publication)
    # ═══════════════════════════════════════════════════════════════════════
    
    # Gelman-Rubin R-hat threshold (< 1.01 for publication quality)
    'rhat_threshold': 1.01,
    
    # R-hat threshold for quick testing
    'rhat_threshold_test': 1.1,
    ...
    # Maximum autocorrelation time threshold
    'autocorr_threshold': 50,
    
    # ═══════════════════════════════════════════════════════════════════════
    # Adaptive Proposal Settings
    # ═══════════════════════════════════════════════════════════════════════
    'adaptive_proposal': {
        'initial_scale': 0.01,      # Initial proposal scale
        'target_acceptance': 0.234, # Optimal for Gaussian
        'adaptation_window': 100,   # Steps between adaptations
        'max_scale': 0.1,           # Maximum proposal scale
    },
    ...
    # ═══════════════════════════════════════════════════════════════════════
    'convergence_checks': {
        'check_interval': 1000,     # Check convergence every 1000 steps
        'required_checks': 4,       # Require 4 consecutive checks
        'max_steps': 100000,        # Absolute maximum steps
    },
    
    # ═══════════════════════════════════════════════════════════════════════
    # Backend & Storage
    # ═══════════════════════════════════════════════════════════════════════
    'backend': 'hdf5',              # HDF5 for large chains
    'compression': 'gzip',          # Compress to save space
    
    # ═══════════════════════════════════════════════════════════════════════
    # Diagnostics
    # ═══════════════════════════════════════════════════════════════════════
    'monitor_acor': True,           # Monitor autocorrelation
    'monitor_acceptance': True,     # Track acceptance rate
    'monitor_divergences': True,    # Track divergences
