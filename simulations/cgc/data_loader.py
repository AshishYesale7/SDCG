...
def compute_cgc_predictions(data: Dict[str, Any], params: CGCParameters) -> Dict[str, np.ndarray]:
    ...
    mu = params.cgc_mu
    n_g = params.n_g  # Fixed theoretical value = 0.0125
...
class DataLoader:
    ...
    def _generate_mock_data(self, include_sne: bool, 
        ...
        Dl_lcdm = (5000 * np.exp(-((ell - 220)/80)**2) +
                   ...
                   1000 * np.exp(-((ell - 800)/120)**2) +
                   300 * np.exp(-ell/1500) + 100)
        Dl_cgc = Dl_lcdm * (1 + params.cgc_mu * (ell/1000)**(params.n_g/2))
        ...
        bao_cgc = bao_true * (1 + params.cgc_mu * (1 + bao_z)**(-params.n_g))
        ...
        fs8_cgc = fs8_lcdm * (1 + 0.1 * params.cgc_mu * (1 + z_growth)**(-params.n_g))
