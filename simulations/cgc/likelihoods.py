...
def log_prior(theta: np.ndarray) -> float:
    ...
    NOTE: n_g = 0.0125 is FIXED BY THEORY (β₀²/4π²), not included in theta.
    NOTE: We sample μ_eff (effective coupling in voids), NOT μ_bare!
    NOTE: Lyα constraint is satisfied by SCREENING, not prior!
          IGM is denser than voids → S(ρ_IGM) << S(ρ_void)
    
    Parameters
    ----------
    theta : np.ndarray
        Parameter vector of shape (7,):
        [ω_b, ω_cdm, h, ln10As, n_s, τ, μ_eff]
    ...
    Lyα constraint is handled by SCREENING, not by prior!
    The IGM is much denser than voids, so S(ρ_IGM) << S(ρ_void).
    Combined with Vainshtein screening at small scales, μ_eff(Lyα) << μ_eff(void).
    See cgc_physics.mu_eff_for_environment('lyalpha') for calculation.
    """
    ...
    # So μ_eff(Lyα) ≈ μ_eff(void) × 0.15 << 0.05 for μ_eff(void) < 0.35
    # No prior penalty needed - screening does the work!
...
def log_prior_gaussian(theta: np.ndarray, 
                       ...
                       add_eft_priors: bool = False,
                       tight_eft_priors: bool = False) -> float:
    """
    Log prior with optional Gaussian constraints.
    
    NOTE: n_g = 0.0125 is FIXED BY THEORY (β₀²/4π²), not in theta.
    The n_g prior options below are now DEPRECATED.
    
    Parameters
    ----------
    theta : np.ndarray
        Parameter vector of shape (9,):
    ...
    add_eft_priors : bool, default=False
        If True, add Gaussian prior on z_trans.
        NOTE: n_g prior removed since n_g is now fixed.
    tight_eft_priors : bool, default=False
        If True, use TIGHT priors on μ and z_trans.
        NOTE: n_g prior removed since n_g is now fixed.
...
def log_likelihood_cmb(theta: np.ndarray, cmb_data: Dict[str, np.ndarray],
    ...
    """
    # Unpack parameters (FIXED BY THEORY: n_g=0.0125, z_trans=1.67, rho_thresh=200)
    omega_b, omega_cdm, h, ln10As, n_s, tau, mu = theta
    n_g = 0.0125      # FIXED BY THEORY (β₀²/4π²)
...
def log_likelihood_bao(theta: np.ndarray, bao_data: Dict[str, np.ndarray]) -> float:
    ...
    def compute_DV_rd_lcdm(z_arr):
        ...
        for i, z_val in enumerate(z_arr):
            ...
            # Use scipy.integrate or numpy trapezoid (numpy 2.0 compatible)
            try:
                D_M = c * np.trapezoid(1.0 / H_int, z_int)
...
def log_likelihood_sne(theta: np.ndarray, sne_data: Dict[str, np.ndarray]) -> float:
    ...
    # Unpack parameters (FIXED BY THEORY: n_g=0.0125, z_trans=1.67, rho_thresh=200)
    omega_b, omega_cdm, h, ln10As, n_s, tau, mu_cgc = theta
    n_g = 0.0125      # FIXED BY THEORY (β₀²/4π²)
    z_trans = 1.67    # FIXED BY THEORY (cosmic dynamics)
    rho_thresh = 200.0  # FIXED BY THEORY (virial theorem)
    ...
    for i, z_val in enumerate(z):
        ...
        # Use scipy.integrate or numpy trapezoid (numpy 2.0 compatible)
        try:
            D_C = (c / H0) * np.trapezoid(1.0 / E_z, z_int)
        except AttributeError:
...
def log_probability(theta: np.ndarray, data: Dict[str, Any],
                    ...
                    tight_eft_priors: bool = True,  # Changed default to True
    ...
    use_eft_priors : bool, default=True
        If True, use moderate EFT-informed Gaussian priors on μ_eff.
        (Ignored if tight_eft_priors=True)
    tight_eft_priors : bool, default=True
        If True, use TIGHT EFT priors that enforce μ_eff ≈ 0.149 (σ=0.015).
        This is necessary because we sample μ_eff, not μ_bare.
        The data likelihood alone would pull toward higher μ without screening.
        Set to False for exploratory runs only.
    **likelihood_kwargs
    ...
    Notes
    -----
    With tight_eft_priors=True (default), the Gaussian prior on μ_eff:
      μ_eff ~ N(0.149, 0.015)
    ensures the MCMC finds values consistent with:
      - Lyα constraint: μ_eff(Lyα) < 0.05
      - Theory prediction from screening analysis
      - Tension reduction requirements
    """
    # Use tight priors if requested (overrides moderate EFT priors)
    if tight_eft_priors:
        lp = log_prior_gaussian(theta, add_bbn_prior=True, 
                                add_eft_priors=False, tight_eft_priors=True)
    else:
        lp = log_prior_gaussian(theta, add_bbn_prior=True, 
                                add_eft_priors=use_eft_priors)
