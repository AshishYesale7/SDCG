...
class CGCTheory:
    ...
    Examples
    --------
    >>> theory = CGCTheory(mu=0.47, n_g=0.014)
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    # THESIS v12 CANONICAL CGC PARAMETERS (Technical Supplement)
    # All values derived from fundamental physics
    # ═══════════════════════════════════════════════════════════════════════════
    mu: float = 0.47          # MCMC best-fit (±0.03) - THE ONLY FREE PARAMETER
    n_g: float = 0.0125       # FIXED: β₀²/(4π²) = 0.70²/39.48
    ...
    sigma_z: float = 1.5      # Width of Gaussian redshift window
    ...
    def G_eff_ratio(self, k: float = 0.1, z: float = 0.0) -> float:
        """
        Compute ratio G_eff / G_N - THESIS v12 CANONICAL FORMULA.
        
        Master Equation (Thesis v12):
        ...
        # THESIS v12: Gaussian redshift evolution
        # g(z) = exp[-(z - z_trans)²/(2σ_z²)]
        g_z = np.exp(-0.5 * ((z - self.z_trans) / self.sigma_z) ** 2)
    ...
    def E_z(self, z: Union[float, np.ndarray]) -> Union[float, np.ndarray]:
        """
        Compute E(z) = H(z) / H0 in CGC cosmology - THESIS v12.
        ...
        # THESIS v12: CGC modification with Gaussian g(z)
        # g(z) = exp[-(z - z_trans)²/(2σ_z²)]
        g_z = np.exp(-0.5 * ((z - self.z_trans) / self.sigma_z) ** 2)
